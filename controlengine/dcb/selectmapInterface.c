...
#else //DCSC_TEST
const char* g_pDefaultSmDevice=(Ctr::tmpDir+"/virtex").c_str();
#endif //DCSC_TEST
...
uint32_t smMakeT1Header(uint8_t rdwr, uint16_t address, uint16_t words)
{
...
uint32_t smMakeT2Header(uint8_t rdwr, uint32_t words)
{
	if (rdwr > 1)
		return 0;
...
uint32_t smMakeFacHeader(uint8_t ba, uint8_t mja, uint8_t mna)
{
	/* checking type, either 0 (write) or 1 (read) */
	if (ba > 2)
		return 0;
...
int smRegisterWrite(uint32_t address, uint32_t pData)
{
	int iResult=0;
...
int smRegisterRead(uint32_t address, uint32_t *pData)
{
	int iResult=0;
