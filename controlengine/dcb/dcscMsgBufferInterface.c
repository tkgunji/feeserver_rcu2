...
const char* g_pDeviceName="/dev/sample2";
#else //DCSC_TEST
 ...
 */
static uint32_t message_in_buffer_size=0x40;  // size of the Message In Buffer (MIB)
static uint32_t message_out_buffer_size=0x40; // size of the Message Result Buffer (MRB)
static uint32_t message_regfile_size=0x10;    // number of control registers
static uint32_t message_out_buffer_offset=0;  // offset of the MRB, set during initialization
static uint32_t regfile_offset=0;             // offset of the control registers, set during initialization
...
uint32_t mkFrstWrd(unsigned short numWords, unsigned short blockNum, unsigned short format, unsigned short mode, unsigned short cmdId){ 
...
uint32_t mkLstWrd(unsigned short checksum){
  uint32_t lstWrd = 0;
 if (g_pMsgBuf_header) {
...
int openDevice(const char* pDeviceName)
{
  ...
  if (g_file>0){
    ...
    pBuffer = (uint32_t*)malloc(bufferSize*4); 
    pMib=pBuffer;
...
int write_dcsc(const void *ptr, size_t size, size_t nmemb)
{
  ...
  if (g_file>0) {
    ...
    if (result>=0)
      ...
      fprintf(stderr, "write_dcsc: internal error: can not write (%i)\n", (int)result);
      iResult=-1;
    }
  } else {
    iResult=-EBADF;
 ...
 *    <0 in case of error
 */
int readResultBuffer(unsigned char* pTargetBuffer, int iTargetBufferSize, uint32_t iStartOffset)
{
  int iResult=0;
 ...
 * internal function
 */
int getErrorFromStatus(uint32_t u32Status)
{
  int iResult=0;
 ...
 *    <0 in case of error
 */
int getCmdResult(uint32_t u32CmdWord, const char* pMessage)
{
  int iNofWords=0;
  uint32_t data[2];
  if ((g_dcscFlags&DCSC_INIT_ENCODE)==0) {
    if (readResultBuffer((unsigned char*)&data, 2*sizeof(uint32_t), 0)>=0) {
      if (g_options&PRINT_RESULT_BUFFER) {
	printBufferHex((unsigned char*)&data, 2*sizeof(uint32_t), 4, "result buffer");
      }
	...
	if (data[1]==0) { // all bits in status word cleare if no error
	  ...
	  if (iNofWords<2) {
	    ...
	    iNofWords=-EIO;
	  } else if (iNofWords>(int)(message_out_buffer_size/sizeof(uint32_t))) {
...
int sendRcuCommand(char* pCmdBuffer, int iCmdBufferSize, int iTimeout)
{
  ...
  if (pCmdBuffer && iCmdBufferSize>0){
    ...
    int bFlash=((*((uint32_t*)pCmdBuffer)>>g_pMsgBuf_header->FRSTWORD_BITSHIFT_MODE)&((0x1<<g_pMsgBuf_header->FRSTWORD_WIDTH_MODE)-1))&MSGBUF_MODE_FLASH;
 ...
 * 
 */
int rcuSingleWriteExt(uint32_t address, uint32_t data, unsigned short mode){
  int iResult=0;
  if (pMib && mibSize>=6) {
  ...
  pMib[4] = mkEndMarker();
  lock_device();
  iResult=sendRcuCommand((char*)pMib, 5*4, 0);
 ...
 * 
 */
int rcuMultipleWriteExt(uint32_t address, uint32_t* pData, int iSize, int iDataMode, unsigned short mode){
  int iResult=0;
  if (pMib && mibSize>=6) {
  if (pData==NULL) {
  ...
  uint32_t* arrayCmdWords=pMib+1;
  // calculate the maximum possible number of data words
  int iMaxDataWordsPerBlock=message_in_buffer_size/sizeof(uint32_t)-5;
  ...
  for (;iBlockNo<iNofBlocks && iResult>=0; iBlockNo++) {
	...
	iResult=-ENOSYS;
      ...
      for(uint32_t i=0; i<arrayCmdWords[1]; i++) {
	int lsbOffset=0;
	int msbOffset=1;
	if (iDataMode==-2) {
	  lsbOffset=1;
...
/*      // check if there were stucking bits in other bytes of the block
      for (i=0; i<arrayCmdWords[1]; i++) {
        uint32_t testPattern=0xffff0000;
        if (iDataSize==1)
      ...
      uint32_t* pSrc=pData+iBlockNo*iMaxDataWordsPerBlock/iCompFactor;
	...
	swab(pSrc, &arrayCmdWords[2], iBulkCopy*sizeof(uint32_t));
      } else {
	memcpy (&arrayCmdWords[2], pSrc, iBulkCopy*sizeof(uint32_t));
      }
      if (iRemCopy!=0) {
	iPayloadSize++;
	...
	memset(((int*)&arrayCmdWords[2+iBulkCopy])+iRemCopy, 0, sizeof(uint32_t)-iRemCopy);
      }
    }
    ...
    if ((g_bCompression==1 && iDataMode<3*switchBigEndianConversion) || iDataMode==-4) {
      ...
      uint16_t tmp=0;
      uint16_t* pLsb=(uint16_t*)&arrayCmdWords[2];
      uint16_t* pMsb=((uint16_t*)&arrayCmdWords[2]); pMsb++;
      int i=0;
	...
	*pMsb=tmp; pMsb+=2;
    ...
    if (iResult>=0) {
      ...
      lock_device();
      iResult=sendRcuCommand((char*)pMib, (iPayloadSize+5)*4,0);
      if(iResult==-ETIMEDOUT){
        iResult=-1;
 ...
 * 
 */
int rcuSingleReadExt(uint32_t address, uint32_t* pData, unsigned short mode)
{
  int iResult=0;
  if (pMib && mibSize>=6) {
  ...
  pMib[3]=mkEndMarker();
  lock_device();
  iResult=sendRcuCommand((char*)pMib, 4*4, 0);
  if(iResult==-ETIMEDOUT){
    iResult=-1;
	...
	if((readResultBuffer((unsigned char*)pData, sizeof(uint32_t), 2*sizeof(uint32_t)))>=0){
	  if(g_options&PRINT_RESULT_BUFFER)
	    printBufferHex((unsigned char*)pData, sizeof(uint32_t), 4, "data buffer");
	} else {
	  if(pData){
 ...
 * 
 */
int rcuMultipleReadExt(uint32_t address, int iSize, uint32_t* pData, unsigned short mode)
{
  int iResult=0;
  int iRead=0;
  if (pMib && mibSize>=6) {
//  char* pCmdBuffer=NULL;
  ...
  int iMaxDataWordsPerBlock=message_out_buffer_size/sizeof(uint32_t)-2;
...
//    iCmdBufferSize=wrapBlock(MULTI_READ, 0, 4, arrayCmdWords, &pCmdBuffer);
	...
	if((iResult=getCmdResult(*(unsigned char*)pMib, "rcuMultipleRead"))>0 && pData){
	  if(readResultBuffer((unsigned char*)(pData+iBlockNo*iMaxDataWordsPerBlock), iResult*sizeof(uint32_t), 2*sizeof(uint32_t))>=0){
	    iRead+=iResult;
	    if(g_options&PRINT_RESULT_BUFFER)
	      printBufferHex((unsigned char*)(pData+iBlockNo*iMaxDataWordsPerBlock), iResult*sizeof(uint32_t), 4, "data buffer");
	  } else {
	    if(pData){
 ...
 * 
 */
int rcuMultipleWrite(uint32_t address, uint32_t* pData, int iSize, int iDataSize){
 ...
 * 
 */
int rcuMultipleRead(uint32_t address, int iSize, uint32_t* pData)
{
 ...
 * not yet implemented
 */
int dcscProvideMessageBuffer(uint32_t** ppBuffer, int* pSize)
{
  int iResult=-ENOSYS;

  //*ppBuffer=pMib;
  //Note that mibSize is uint32_t, pSize is int. Must be fixed.
  //*pSize=mibSize;
  //iResult=1;
 ...
 * not yet implemented
 */
int dcscPrepareMessageBuffer(uint32_t** ppBuffer, int* pSize, unsigned int cmdID, unsigned int flags)
{
  int iResult=-ENOSYS;
  /*
  ...
  - return pointer to first command word location, i.e. start + sizeof(uint32_t)
  */
  return iResult;
}
 ...
 * not yet implemented
 */
int dcscExecuteCommand(uint32_t* pBuffer, int iSize, uint32_t** ppTarget, int* ppTargetSize, unsigned int operation)
{
  int iResult=-ENOSYS;
  /*
  - cross check the buffer, must either be the start or start + sizeof(uint32_t)
...
int dcscCheckMsgBlock(int* pBuffer, int iSize, int iVerbosity) {
  int iResult=0;
...
int rcuFlashWrite(uint32_t address, uint32_t* pData, int iSize, int iDataSize)
{
  int iResult=0;
  if (pData && iSize>0) {
    lock_device();
...
int rcuFlashRead(uint32_t address, int iSize,uint32_t* pData)
{
  int iResult=0;
  if (pData && iSize>0) {
  ...
  if (command>0 && iResult>=0) {
	...
	if (pMib && mibSize>=6) {
	  ...
	  if (command==RCU_FLASH_ERASEALL) {
	    iTimeOut=120; //Increased to support the new Macronix Flash memory chip
	    if (g_verbosity>0)
	  ...
	  pMib[k++]=mkLstWrd(0);
	  pMib[k++]=mkEndMarker();
	  int iResult=sendRcuCommand((char*)pMib, k*4, iTimeOut);
	  if(iResult==-ETIMEDOUT){
	    iResult=-1;
...
int rcuFlashID()
{
  ...
  if (checkState(eFlash, 3)) {
	...
	pMib[k++]=mkLstWrd(0);
	pMib[k++]=mkEndMarker();
	int iResult=sendRcuCommand((char*)pMib, k*4, 0);
	if(iResult==-ETIMEDOUT){
	  iResult=-1;
	  ...
	  if(iResult>=0) {
	    if((iResult=getCmdResult(*(unsigned char*)pMib, "rcuFlashID"))>0){
	      uint32_t id=0;
	      if((readResultBuffer((unsigned char*)&id, sizeof(uint32_t), 1*sizeof(uint32_t)))>=0){
		iResult=id;
		if(g_options&PRINT_RESULT_BUFFER)
		  printBufferHex((unsigned char*)&id, sizeof(uint32_t), 4, "data buffer"); 
	      } else {
		fprintf(stderr,"rcuSingleRead: failed to read data from result buffer\n");
		iResult=-EIO;
	      }
